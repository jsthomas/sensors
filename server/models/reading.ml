...
type t = { sensor_id : int; occurred : Time.date; readings : readings }
[@@deriving yojson]

let empty (s : Sensor.t) occurred =
  let n = Time.readings_per_day 900 in
  (* FIXME: Sensor step sizes. *)
...
let read_range =
  ...
  fun user_id sensor_id start_d end_d (module Db : DB) ->
    ...
    let convert (d, readings) = { sensor_id; occurred = d; readings } in
...
let read_day =
  ...
  fun sensor_id d (module Db : DB) ->
    ...
    let%lwt tuples = Caqti_lwt.or_fail result in
    let convert readings = { sensor_id; occurred = d; readings } in
