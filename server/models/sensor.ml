...
let get =
  ...
  in
  fun user_id sensor_id (module Db : DB) ->
    Db.find_opt query (user_id, sensor_id) >>= Caqti_lwt.or_fail
    >>= fun details_or_error ->
    Lwt.return
      (Option.map
         (fun (name, description, api_key, step) ->
...
let from_key =
  ...
  fun api_key (module Db : DB) ->
    Db.collect_list query api_key >>= Caqti_lwt.or_fail
    >>= fun details_or_error ->
    Lwt.return
      (List.map
         (fun (id, name, description, step) ->
           { id; name; description; api_key; step })
         details_or_error)
