...
let login =
  let login_base login_doc request =
    let%lwt user_id =
      Dream.sql request (Models.User.get login_doc.username login_doc.password)
    in
    match user_id with
    | Some id ->
...
type sensor_create_doc = { name : string; description : string; step : int }
[@@deriving yojson]
...
let date_query_param key ~default request =
  Dream.query key request >>=? Time.date_of_string_opt |> Option.value ~default
...
let get_readings sensor_id request =
  ...
  let%lwt readings =
    Dream.sql request (Reading.read_range user_id sensor_id start_d end_d)
  in
  `List (List.map Reading.yojson_of_t readings) |> json_response

type readings_create_doc = { occurred : Time.date; readings : Reading.readings }
[@@deriving yojson]
  ...
  let write create_obj request =
    ...
    | Some sensor ->
        ...
        if actual = expected then
          ...
          let message =
            Printf.sprintf
              "Incorrect number of readings. Actual: %d, Expected: %d" actual
              expected
          in
